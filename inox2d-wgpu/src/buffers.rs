...
pub struct BufferManager {
    ...
    pub composite_vertex_offset: i32,
...
impl BufferManager {
    ...
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("inox2d_vertex_buffer"),
        ...
        let index_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("inox2d_index_buffer"),
            size: index_capacity,
    ...
    pub fn update(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, puppet: &Puppet) {
        if let Some(render_ctx) = &puppet.render_ctx {
            ...
            let v_start = self.vertex_staging.len() as i32;
            self.vertex_staging.push(Vertex { position: [-1.0, 1.0], uv: [0.0, 0.0], deform: [0.0, 0.0] }); // TL
            self.vertex_staging.push(Vertex { position: [-1.0, -1.0], uv: [0.0, 1.0], deform: [0.0, 0.0] }); // BL
            self.vertex_staging.push(Vertex { position: [1.0, -1.0], uv: [1.0, 1.0], deform: [0.0, 0.0] }); // BR
            self.vertex_staging.push(Vertex { position: [1.0, 1.0], uv: [1.0, 0.0], deform: [0.0, 0.0] }); // TR
            ...
            let base = v_start as u16;
            let quad_indices = [base, base + 1, base + 2, base, base + 2, base + 3];
            self.index_staging.extend_from_slice(bytemuck::cast_slice(&quad_indices));

            // Update Offsets
            self.composite_vertex_offset = v_start;
            
            // Fix: composite_index_offset must account for padding.
            ...
            if required_vertex_size > self.vertex_capacity {
                self.vertex_capacity = required_vertex_size.max(self.vertex_capacity * 2);
            ...
            if required_index_size > self.index_capacity {
                self.index_capacity = required_index_size.max(self.index_capacity * 2);
