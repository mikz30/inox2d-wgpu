...
impl TextureManager {
    ...
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, model: &Model) -> Self {
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("inox2d_texture_bind_group_layout"),
            entries: &[
        ...
        for (i, shallow_tex) in shallow_textures.iter().enumerate() {
            ...
            let texture = device.create_texture(&wgpu::TextureDescriptor {
                label: Some(&format!("inox2d_texture_{}", i)),
                size,
                mip_level_count: 1,
                sample_count: 1,
            ...
            let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
                label: Some(&format!("inox2d_sampler_{}", i)),
            ...
            let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some(&format!("inox2d_bind_group_{}", i)),
